...
export function RecommendationList() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (querySnapshot) => {
      setLoading(true);
      const recs: Recommendation[] = await Promise.all(querySnapshot.docs.map(async (d) => {
          const data = d.data();
          const fromUserDoc = await getDoc(doc(db, 'users', data.fromUserId));
          const fromUserEmail = fromUserDoc.exists() ? fromUserDoc.data().email : 'A friend';
          return {
            id: d.id,
            ...data,
            fromUserEmail,
      ...
      {recommendations.length > 0 && (
        <div className="w-full max-w-4xl space-y-4">
          {recommendations.map((item) => {
            ...
            const href = `/search/${item.mediaType}/${item.movieId}?title=${encodeURIComponent(title)}&poster=${item.moviePoster}&rating=0`;
            const posterUrl = item.moviePoster && item.moviePoster !== 'null' ? `https://image.tmdb.org/t/p/w200${item.moviePoster}` : 'https://placehold.co/200x300.png';
            return (
                    ...
                    <div className="p-4 flex flex-col justify-center gap-2">
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <Avatar className="h-6 w-6">
                                <AvatarFallback>{getInitials(item.fromUserEmail || '?')}</AvatarFallback>
                            </Avatar>
                            <span>
                                <span className="font-semibold text-foreground">{item.fromUserEmail}</span> recommended
                            </span>
                        </div>
