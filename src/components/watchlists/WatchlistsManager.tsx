...
export function WatchlistsManager() {
  ...
  // Load watchlists when user is ready
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        ...
        toast({
          variant: 'destructive',
          title: 'Error',
          description: 'Could not load watchlists.',
        });
        setLoading(false);
      }
    );
    ...
    return () => unsubscribe();
  }, [firebaseUser?.uid, authLoading, toast]);

  // Create new watchlist
  const handleCreateList = async () => {
    if (!firebaseUser?.uid || !newListName.trim()) {
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'User not authenticated or list name empty.',
      });
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Success!', description: `Created list "${newListName.trim()}".` });
      setNewListName('');
  ...
  const handleUpdateName = async () => {
    if (!firebaseUser?.uid || !editingList || !editedName.trim()) return;
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'Could not update list name.',
      });
    } finally {
      setIsUpdating(false);
    }
  ...
  const handleDeleteList = async () => {
    if (!firebaseUser?.uid || !deletingList) return;
    ...
    <div className="space-y-8">
      ...
      {watchlists.length === 0 ? (
        <Alert>
          ...
          <AlertDescription>
            Create your first watchlist to start saving movies and TV shows. Use the "New List"
            button.
          </AlertDescription>
        </Alert>
      ) : (
                ...
                <div className="flex gap-2">
                  <Dialog onOpenChange={(open) => !open && setEditingList(null)}>
                    ...
                    {editingList?.id === list.id && (
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>Rename "{editingList.name}"</DialogTitle>
                        </DialogHeader>
                        <div className="flex gap-2 py-4">
                  ...
                  <Dialog onOpenChange={(open) => !open && setDeletingList(null)}>
                    ...
                    {deletingList?.id === list.id && (
                      <DialogContent>
                        ...
                        <DialogFooter>
                          <Button variant="secondary" onClick={() => setDeletingList(null)}>
                            Cancel
                          </Button>
